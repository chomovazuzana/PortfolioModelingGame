...
describe('calculateYearResult', () => {
    it('computes 2021 result for worked example', () => {
        const result = calculateYearResult({ cash: 5, bonds: 10, equities: 40, commodities: 25, reits: 20 }, 2021, 100_000);
    ...
    it('preserves capital with 100% cash in 2021', () => {
        const result = calculateYearResult({ cash: 100, bonds: 0, equities: 0, commodities: 0, reits: 0 }, 2021, 100_000);
        expect(result.valueEnd).toBeCloseTo(100_100, 0);
    });
    it('throws for invalid year', () => {
        expect(() => calculateYearResult({ cash: 100, bonds: 0, equities: 0, commodities: 0, reits: 0 }, 2020, 100_000)).toThrow('No return data for year 2020');
    });
    it('computes correct breakdown amounts', () => {
        const result = calculateYearResult({ cash: 20, bonds: 20, equities: 20, commodities: 20, reits: 20 }, 2021, 100_000);
