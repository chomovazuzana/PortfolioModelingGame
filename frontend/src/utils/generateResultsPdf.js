export async function generateResultsPdf(results, gameName, playerName) {
    ...
    doc.text(playerResult.rank > 0
        ? `Rank: ${playerResult.rank} / ${playerResult.totalPlayers}`
        : 'Rank: Unranked (hidden)', 14, 78);
    // Allocations table
    doc.setFontSize(14);
    doc.setTextColor(0);
    ...
    const allocTableEnd = doc.lastAutoTable?.finalY ?? 140;
